...
def _manager_action_matches_stage(stage: str, user_message: str, user_turns: int) -> bool:
    lower = user_message.lower().replace("ё", "е")
    if stage == "intro":
        return (
            any(word in lower for word in ("здрав", "добрый", "меня зовут", "компан"))
            and any(word in lower for word in ("удобно", "пару", "минут", "вопрос", "рассказать"))
            and len(lower) > 35
        )
    if stage == "need_discovery":
        return user_turns >= 1 and ("?" in user_message or any(word in lower for word in ("что важно", "не устраивает", "какие", "как сейчас", "пользуетесь")))
    if stage == "presentation":
        return user_turns >= 1 and any(word in lower for word in ("поможет", "решит", "вы получите", "подойдет", "польз", "эконом", "контрол"))
    if stage == "objection":
        return user_turns >= 1 and any(word in lower for word in ("понимаю", "соглас", "давайте", "сравним", "пилот", "без риска", "уточню"))
    if stage == "closing":
        return user_turns >= 1 and any(word in lower for word in ("оформ", "кп", "демо", "заяв", "встреч", "подключ", "созвон", "завтра"))
    return user_turns >= 2


def _stage_success_markers(stage: str) -> list[str]:
    markers = {
        "intro": ["задавайте", "слушаю", "давайте коротко", "один-два вопроса", "есть пара минут", "готов выслушать"],
        "need_discovery": ["мне важно", "проблема", "хочу чтобы", "нужно", "не хватает", "неудобно", "меня волнует"],
        "presentation": ["звучит интересно", "подходит", "понимаю пользу", "можно обсудить", "это решит", "полезно"],
        "objection": ["логично", "давайте посмотрим", "можно сравнить", "звучит разумно", "готов обсудить", "тогда понятно"],
        "closing": ["оформляйте", "давайте подключим", "пришлите кп", "согласен на демо", "завтра удобно", "давайте заявку", "готов попробовать", "назначим встречу"],
    }
